...
class Model:
    ...
    def encoder(self, inputs):
        with tf.name_scope("encoder") as scope:
            ...
            hidden = tf.layers.dense(hidden1, 32, activation=tf.nn.relu)
        return hidden

    def decoder(self, hidden):
        with tf.name_scope("decoder") as scope:
            outputs1 = tf.layers.dense(hidden, 128, activation=tf.nn.relu)
            outputs = tf.layers.dense(outputs1, 784, activation=tf.nn.sigmoid)
        return outputs
